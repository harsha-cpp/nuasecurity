...
// Load .env.flags if present from project root
const envPath = path.resolve(__dirname, '..', '.env.flags')
...
async function main() {
  ...
  const postsData = [
    ...
    { 
      ...
      createdAt: new Date('2024-06-08T13:30:00Z'),
    ...
    { 
      ...
      category: categories[0],
      createdAt: new Date('2024-07-18T15:45:00Z'),
    ...
    { 
      ...
      category: categories[3],
      createdAt: new Date('2024-10-03T17:30:00Z'),
    ...
    { 
      ...
      category: categories[4],
      createdAt: new Date('2024-11-22T10:15:00Z'),
  ...
  // Ensure predictable IDs for comments (1..3) for easy testing
  await prisma.adminComment.create({ data: { id: 1, comment: 'Remember to review new posts before publishing.', hidden: true } }).catch(() => {})
  await prisma.adminComment.create({ data: { id: 2, comment: 'Consider adding more categories as the blog grows.', hidden: true } }).catch(() => {})
  await prisma.adminComment.create({ data: { id: 3, comment: 'Great engagement on the AI and minimalism posts!', hidden: true } }).catch(() => {})
  ...
  // Add a sample secret for admin functionality
  await prisma.secret.create({ data: { sflag: 'BLOG_ADMIN_SECRET_2024' } })
